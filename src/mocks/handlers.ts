...
export const mockChildren = [
  {
    ...
    created_at: "2024-01-01T00:00:00Z",
    enrollments: [{ id: "enroll-1", status: "active", class_id: "class-1" }],
  },
  {
    id: "child-2",
    first_name: "Jenny",
...
export const handlers = [
  ...
  http.post(
    `${API_BASE}/enrollments/:id/transfer`,
  ...
  http.get(`${API_BASE}/check-in/class/:classId/status`, ({ params }) => {
    const today = new Date().toISOString().split("T")[0];
    return HttpResponse.json({
      class_id: params.classId as string,
      check_in_date: today,
      statuses: mockChildren.map((child, index) => ({
        enrollment_id: `enroll-${index + 1}`,
        is_checked_in: index === 0, // First child checked in
  ...
  http.post(`${API_BASE}/check-in`, async ({ request }) => {
    ...
    return HttpResponse.json({
      ...
      check_in_date:
        data.check_in_date || new Date().toISOString().split("T")[0],
      is_late: false,
      created_at: new Date().toISOString(),
    });
  }),
  ...
  http.get(`${API_BASE}/badges/leaderboard`, () => {
    return HttpResponse.json([{ child: mockChildren[0], badge_count: 5 }]);
  }),
