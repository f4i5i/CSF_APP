...
export default function useClassForm(initialData = null, mode = "create") {
  ...
  const transformPaymentOptionsFromBackend = (backendOptions) => {
    ...
    backendOptions.forEach((backendOpt) => {
      ...
      const index = frontendOptions.findIndex(
        (opt) => opt.type === frontendType,
      );
      ...
      if (index !== -1) {
        frontendOptions[index] = {
          ...
          custom_name:
            backendOpt.custom_name ||
            backendOpt.display_name ||
            defaultNames[frontendType] ||
            backendOpt.name,
        };
      }
    });
  ...
  useEffect(() => {
    if (initialData) {
      ...
      if (
        ...
        Array.isArray(initialData.weekdays) &&
        initialData.weekdays.length > 0
      ) {
        ...
        Array.isArray(initialData.schedule) &&
      ...
      const paymentOptions = transformPaymentOptionsFromBackend(
        initialData.payment_options,
      );
      ...
      const mappedData = {
        ...
        registration_start_date: initialData.registration_start_date
          ...
          : initialData.registration_start
            ? initialData.registration_start.split("T")[0]
            : "",
        registration_end_date: initialData.registration_end_date
          ? initialData.registration_end_date.split("T")[0]
          : initialData.registration_end
            ? initialData.registration_end.split("T")[0]
            : "",
        ...
        class_image:
          initialData.class_image_url || initialData.class_image || null,
  ...
  const validate = () => {
    ...
    const enabledOptions = formData.payment_options.filter(
      (opt) => opt.enabled,
    );
    ...
    if (formData.registration_start_date && formData.registration_end_date) {
      if (
        new Date(formData.registration_end_date) <=
        new Date(formData.registration_start_date)
      ) {
    ...
    if (
      !formData.repeat_every_weeks ||
      parseInt(formData.repeat_every_weeks) < 1
    ) {
  ...
  const handleSubmit = async (onSuccess, overrides = {}) => {
    ...
    try {
      ...
      const { slug: _slug, ...mergedDataWithoutExcluded } = mergedData;
