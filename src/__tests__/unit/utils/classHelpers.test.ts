...
describe("classHelpers utilities", () => {
  ...
  describe("getOfferingType", () => {
    ...
    it('should return "membership" for offering_type containing "member"', () => {
      expect(getOfferingType({ offering_type: "membership" })).toBe(
        "membership",
      );
    });

    it('should return "membership" for subscription type', () => {
      expect(getOfferingType({ class_type: "subscription" })).toBe(
        "membership",
      );
    });
    ...
    it('should return "short-term" for short type', () => {
      expect(getOfferingType({ offering_type: "short-term" })).toBe(
        "short-term",
      );
    });
    ...
    it("should check fields in priority order (string fields first)", () => {
      ...
      expect(
        getOfferingType({ offering_type: "membership", is_short_term: true }),
      ).toBe("membership");
    });
  });
  ...
  describe("getOfferingLabel", () => {
    it('should return "Monthly Membership" for membership type', () => {
      expect(getOfferingLabel("membership")).toBe("Monthly Membership");
    });

    it('should return "One Time Payment" for short-term type', () => {
      expect(getOfferingLabel("short-term")).toBe("One Time Payment");
    });
  ...
  describe("getPriceModelLabel", () => {
    it('should return "Monthly Membership" when no class and offering is membership', () => {
      expect(getPriceModelLabel(undefined, "membership")).toBe(
        "Monthly Membership",
      );
    });

    it('should return "One Time Payment" when no class and offering is short-term', () => {
      expect(getPriceModelLabel(undefined, "short-term")).toBe(
        "One Time Payment",
      );
    });
    ...
    it("should titleCase the pricing_model from class", () => {
      expect(
        getPriceModelLabel({ pricing_model: "monthly_subscription" }),
      ).toBe("Monthly Subscription");
    });

    it("should use price_model field", () => {
      expect(getPriceModelLabel({ price_model: "weekly-plan" })).toBe(
        "Weekly Plan",
      );
    });

    it("should fall back to offering type labels when no string fields found", () => {
      expect(getPriceModelLabel({ is_membership: true })).toBe(
        "Monthly Membership",
      );
    });
  ...
  describe("getDirectRegistrationLink", () => {
    it("should return explicit registration_link when present", () => {
      const link = getDirectRegistrationLink("class-1", {
  ...
  describe("getCancellationSummary", () => {
    it("should return default summary for undefined class", () => {
      ...
      expect(result).toBe(
        "Cancellations require 24 hours notice. Please contact support for help.",
      );
    });

    it("should return cancellation_policy_summary when present", () => {
      const result = getCancellationSummary({
        cancellation_policy_summary: "No refunds after 48 hours.",
      });
    ...
    it("should use cancellation_window to build summary", () => {
      const result = getCancellationSummary({
        cancellation_window: "up to 24 hours before class",
      });
