      ...
      <div className="p-4 space-y-4">
        <div className="flex gap-4">
          <div className="relative w-24 h-24 flex-shrink-0 rounded-xl overflow-hidden border border-gray-200 bg-gray-50">
            ...
            {canShow(cls.display_settings, "show_program_type") &&
              badgeLabel && (
          ...
          <div className="flex-1 space-y-1">
            ...
            <h3 className="text-xl font-semibold text-[#173151] leading-tight">
              {cls.title}
            </h3>
            {cls.programName && (
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
          ...
          {canShow(cls.display_settings, "show_capacity") && (
            <InfoRow label="Capacity" value={capacityLabel} />
          )}
          {canShow(cls.display_settings, "show_pricing") && (
            <InfoRow label="Price" value={priceLabel} />
          )}
          {canShow(cls.display_settings, "show_pricing") && cls.priceModel && (
            <InfoRow label="Price model" value={cls.priceModel} />
          )}
        </div>

        <div className="flex flex-wrap items-center justify-between gap-3">
          {canShow(cls.display_settings, "show_spots_remaining") ? (
            <div>
              <p
                className={`text-sm font-semibold ${isFull ? "text-rose-600" : "text-emerald-600"}`}
              >
                {isFull
                  ? "Waitlist only"
                  : spotsRemaining > 0
