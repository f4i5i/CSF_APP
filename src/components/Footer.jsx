...
const Footer = ({ isFixed = true , mobileHidden=false }) => {
  return (
     <footer className={`${isFixed ? "fixed bottom-0 left-0 right-0" : "relative"} ${mobileHidden ? "max-sm:hidden":"max-sm:block"} w-full sm:py-4 pt-4 transparent backdrop-blur-sm `}>
        <div className="w-full flex flex-row max-sm:flex  max-sm:flex-col-reverse max-sm:gap-2 items-center justify-between px-6 text-gray-600 text-xs sm:text-sm">
          ...
          <div className="flex gap-5">
           <a href="#" className="flex font-['inter']  text-sm text-black items-center gap-1 hover:underline"> <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round"> <rect x="3" y="11" width="18" height="11" rx="2" ry="2"></rect> <path d="M7 11V7a5 5 0 0 1 10 0v4"></path> </svg> Privacy </a>
