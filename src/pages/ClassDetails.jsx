...
export default function ClassDetail() {
  ...
  const scheduleFromClass = classDetail
    ...
    : [];
  const scheduleLabel = formatSchedule(scheduleFromClass);
  ...
  const registrationLink = classDetail
    ? getDirectRegistrationLink(classDetail.id ?? id, classDetail)
    : "";
  const cancellationSummary = getCancellationSummary(classDetail);
  const priceLabel =
    classDetail?.price_display ||
    classDetail?.price_text ||
    (classDetail?.base_price
      ? `$${classDetail.base_price}`
      : "Contact for pricing");
  const scheduleItems = scheduleFromClass;
            ...
            <div className="grid gap-4 md:grid-cols-2">
              ...
              <div className="rounded-2xl bg-white/70 p-6 space-y-3">
                ...
                <p className="text-xs text-gray-500">
                  Share this URL for direct registration.
                </p>
              </div>
            </div>
