  ...
  return (
    <div className=" min-h-screen max-sm:h-fit bg-gradient-to-b from-[#f3f6fb] via-[#dee5f2] to-[#c7d3e7] opacity-8 max-sm:pb-20">
      ...
      <main className=" mx-6 py-8 max-sm:py-2 max-sm:mx-3">
        ...
        <div className="flex flex-wrap gap-6 max-sm:gap-4 max-sm:px-2 py-2 mb-8">
          {badges.map((badge, i) => (
        ...
        <div className="flex flex-wrap gap-6 max-sm:gap-4 pb-12">
          {locked.map((item, index) => (
            <div
              key={index}
              className="bg-white/60 w-[200px] max-sm:w-full max-sm:min-w-[160px] shrink-0 rounded-2xl py-8 flex flex-col items-center justify-center text-center shadow-sm border border-gray-200"
            >
              ...
              <p className="text-xs xxl1:text-sm text-gray-500 mt-1 px-4">
                {item.desc}
              </p>
            </div>
          ))}
